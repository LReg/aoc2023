...
int main(int argc, char const *argv[] ) {
    if (argc != 4) {
        fputs("Usage: main [amountRed] [amountGreen] [amountBlue]", stderr);
        exit(1);
    }
    int amountRed = atoi(argv[1]);
    int amountGreen = atoi(argv[2]);
    int amoutBlue = atoi(argv[3]);
    FILE * input = fopen("day2/1/input.txt", "r");
    if (input == NULL) {
        fputs("Could not read file ./input.txt", stderr);
        exit(1);
    }
    fprintf(stdout, "is it possible for: red: %i, green: %i, blue: %i\n", amountRed, amountGreen, amoutBlue);
    char line[MAXLINE];
    int addUpIds = 0;
    while(fgets(line, MAXLINE, input) != NULL) {
        struct ColorCounts erg = readLine(line);
        printColorCounts(erg);
        if (erg.red <= amountRed && erg.blue <= amoutBlue && erg.green <= amountGreen) {
            addUpIds += erg.gameNr;
        }
    }
    printf("%i\n", addUpIds);
